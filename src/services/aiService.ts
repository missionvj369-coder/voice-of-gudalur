/**
 * aiService.ts — 100% open-source AI layer for Voice of Gudalur.
 *
 * Chat: proxied by the server to any OpenAI-compatible local LLM
 *       (default: Ollama at http://127.0.0.1:11434/v1, model llama3.2).
 *       When the LLM is unreachable (offline Gudalur), the server returns the
 *       official civic-guidance fallback so the UI always has an answer.
 * Weather: Open-Meteo (open data, no API key).
 */
