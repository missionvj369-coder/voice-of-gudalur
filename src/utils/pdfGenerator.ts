...
export const generatePetitionPDF = (petition: Petition) => {
  ...
  doc.setFontSize(16);
  doc.text('ONE GUDALUR CITIZEN ACTION FORUM', margin, 12);
  ...
  doc.text('All supporting signatures are verified against unique Gudalur Resident Digital IDs (GD-XXXX), Revenue Village listings, and OTP verification on the OneGudalur platform.', margin + 4, currentY + 13);
  ...
  doc.text('Generated via OneGudalur Platform • Independent Democratic Civic Record • https://ai.studio/build/onegudalur', margin, 285);

  doc.save(`OneGudalur_Representation_${petition.id}_${new Date().toISOString().slice(0, 10)}.pdf`);
};
