...
export const GrievanceTrackerModal: React.FC<GrievanceTrackerModalProps> = ({ isOpen, onClose, onAdded }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existing = JSON.parse(localStorage.getItem('onegudalur_my_grievances') || '[]');
    existing.unshift(record);
    localStorage.setItem('onegudalur_my_grievances', JSON.stringify(existing));
                ...
                <div className="text-center py-4 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 mt-1">
                      Token <strong className="font-mono text-emerald-800">{complaintId}</strong> is now tracked on your ONE GUDALUR dashboard.
                    </p>
                  </div>
                  <button
                    onClick={handleReset}
