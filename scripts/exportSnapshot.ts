...
async function main() {
  ...
  // Netlify build sandbox (and local dev without DB access) cannot reach
  // CockroachDB. Snapshots are generated by the refresh-snapshots cron
  // function at runtime (which runs in a serverless function WITH db access).
  // The build must NEVER fail because the DB is unreachable at build time.
  // We attempt the DB connection; if it fails, we emit empty snapshots.
  try {
    ...
    // DB unreachable at build time — emit empty snapshots (cron will fill them)
    console.log('  ⚠ DB unreachable at build time — generating empty snapshots (cron will fill them)');
    console.log('    Error:', e?.message?.split('\n')[0] || e);
    await Promise.all([
      writeJson(path.join(OUT, 'stats.json'), { total: 0, gudalur: 0, outside: 0, places: [], updatedAt: new Date().toISOString() }),
      writeJson(path.join(OUT, 'ledger.json'), { signs: [], updatedAt: new Date().toISOString() }),
      writeJson(path.join(OUT, 'media.json'), { media: [], total: 0, updatedAt: new Date().toISOString() }),
    ]);
    console.log('Done — empty snapshots (will be populated by cron)');
  }
}
