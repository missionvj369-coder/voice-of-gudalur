...
async function main() {
  ...
  // Check if DATABASE_URL is available — Netlify build sandbox cannot reach
  // CockroachDB, so snapshots are generated by the refresh-snapshots cron
  ...
  if (!hasDb) {
    console.log('  ⚠ DATABASE_URL not set — generating empty snapshots (cron will fill them)');
    const empty = { total: 0, signs: [], media: [], updatedAt: new Date().toISOString() };
    await Promise.all([
      writeJson(path.join(OUT, 'stats.json'), { total: 0, gudalur: 0, outside: 0, places: [], updatedAt: new Date().toISOString() }),
      writeJson(path.join(OUT, 'ledger.json'), empty),
